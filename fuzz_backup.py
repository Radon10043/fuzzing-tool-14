...
def checkCrash(testcase, returncode):           #这里的检测缺陷方法是否合适？
    global crash_code
    if returncode != 0:
...
def getFitness(testcase,targetSet,program_loc,callGraph,maxTimeout):
    ...
    crashResult = checkCrash(testcase,p.returncode)
    distance = 0
...
def fuzz(source_loc,ui,uiFuzz,fuzzThread):
    ...
    # p=Popen([program_loc],stdout=PIPE,stdin=PIPE,stderr=STDOUT)
    # try:
    #     info = p.communicate(input=bytes("120",encoding="utf8"),timeout=100)
    #     out = info[0]
    # except TimeoutExpired:
    #     out = b"timeout"
    #     print("超时")
    ...
    count_crash = 1         # 标记要保存的能使程序异常退出的测试用例
    ...
    targetSet = ui.targetSetInfo.toPlainText().split("\n")
    if len(targetSet) == 0:
        print("No target!")
        return
    ...
    while eval(condition):
        ...
        for i in range(0,len(testcase)):
            ...
            crash = returnData[4]
            if crash:
                crashFile = open(now_loc + "\\out\\crash\\crash" + str(count_crash) + ".txt", mode="w")
        ...
        fuzzInfo += "缺陷数量\t\t\t"+str(len(crash_code))+"\n"
...
crash_code = []         # 存储异常退出导致的错误代码
                        # 这里不太合适，改为触发错误覆盖到了哪些结点，如果覆盖到crashNode中没有的结点，就将该结点
