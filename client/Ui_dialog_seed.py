...
class Ui_Dialog(object):
    ...
    def setTableContent(self):
        ...
        for key, val in structDict.items():
            ...
            for key, val in val.items():
                ...
                #         val["lower"], val["upper"])
                self.structTable.setCellWidget(i, 2, self.lineEditItem(True, val["value"], "value", structKey, key))  # 当前值
                self.structTable.setCellWidget(i, 3, self.lineEditItem(True, val["lower"], "lower", structKey, key))  # 下限
                self.structTable.setCellWidget(i, 4, self.lineEditItem(True, val["upper"], "upper", structKey, key))  # 上限
                self.structTable.setItem(i, 5, self.enableeditItem(str(val["bitsize"])))  # 位
                self.structTable.setItem(i, 6, self.enableeditItem(str(val["comment"])))  # 注释
    ...
    def varCheckBoxItem(self, checkBool, struct, memVal):
        ...
        checkBox.stateChanged.connect(lambda: self.varCheckChange(checkBox.isChecked(), struct, memVal))
        return checkBox
    ...
    def checkCodeCheckBoxItem(self, checkBool, struct, memVal):
        ...
        checkBox = QtWidgets.QCheckBox()
        checkBox.setChecked(checkBool)
        checkBox.stateChanged.connect(lambda: self.checkCodeCheckChange(checkBox.isChecked(), struct, memVal))
    ...
    def checkFieldCheckBoxItem(self, checkBool, struct, memVal):
        ...
        checkBox.stateChanged.connect(lambda: self.checkFieldCheckChange(checkBox.isChecked(), struct, memVal))
        return checkBox
    ...
    def lineEditItem(self, isNumber, placeholderText, whatThing, struct, memVal):
        ...
        if whatThing == "value" and placeholderText == None:
            ...
            if "float" in dataType or "double" in dataType:
                structDict[struct][memVal]["value"] = self.getRanFloatNum(structDict[struct][memVal]["lower"], structDict[struct][memVal]["upper"])
                lineEdit.setPlaceholderText("随机值(%.2f)" % structDict[struct][memVal]["value"])  # 浮点型默认文字
            else:
                structDict[struct][memVal]["value"] = self.getRanIntNum(structDict[struct][memVal]["lower"], structDict[struct][memVal]["upper"])
                lineEdit.setPlaceholderText("随机值(%d)" % structDict[struct][memVal]["value"])  # 整型默认文字
        else:
    ...
    def editFinish(self, text, whatThing, struct, memVal, lineEdit):
        ...
        if text != "" and whatThing == "value":
            ...
            if float(text) <= structDict[struct][memVal]["upper"] and float(text) >= structDict[struct][memVal]["lower"]:
                ...
                # 超范围错误提醒-start
                msg_box = QMessageBox(QMessageBox.Warning, "错误", "请输入%s-%s内的值" % (structDict[struct][memVal]["lower"], structDict[struct][memVal]["upper"]))
                msg_box.exec_()
                lineEdit.clear()
                # 超范围错误提醒-end
    ...
    def genMutate(self):
        """生成一个c文件，里面有变异的方法和将值设置在指定范围内的方法
        ...
        for key in structDict:
            struct = key
        try:
            public.genMutate(self.header_loc_list, struct, structDict, self.checkCodeComboBox.currentText(), hasCheckCode)
            print("mutate.c生成成功!")
        except BaseException as e:
            print("\033[1;31m")
            traceback.print_exc()
    ...
    def gen_check_code(self, structDict, struct):
        ...
        elif (check_code_holder_name == 0 and len(check_code_value) != 0) or (check_code_holder_name != 0 and len(check_code_value) == 0):
